...
static void telnet_auto_complete(struct connection *connection)
{
	...
	while ((usr_cmd_pos < t_con->line_cursor) && isspace((signed char)t_con->line[usr_cmd_pos]))
		usr_cmd_pos++;
	...
	for (size_t i = 0; i < usr_cmd_len; i++) {
		if ((i < usr_cmd_len - 1) && isspace((signed char)t_con->line[usr_cmd_pos + i])
				&& isspace((signed char)t_con->line[usr_cmd_pos + i + 1])) {
			optimized_spaces++;
			continue;
		}
